...
function startNewGame() {
    alert("Permainan Baru Dimulai! Tebak Kata Baru :)");
    // Hide hint element
...
function handleInput(e) {
    ...
    if (correctLetters.length === word.length) {
        alert(`Selamat! Kamu Menemukan Kata tersebut ${word.toUpperCase()}`);
        startNewGame();
    } else if (maxGuesses < 1) {
        alert("Permainan Berakhir! Kamu Tidak Punya Tebakan Tersisa!");
        for (let i = 0; i < word.length; i++) {
